...
BOOST_AUTO_TEST_CASE(blockhash)
{
	...
	auto blockhashFun = TypeProvider::get().functionType(strings{"uint256"}, strings{"bytes32"},
...
BOOST_AUTO_TEST_CASE(gas_left)
{
	...
	bytes code = compileFirstExpression(
		sourceCode, {}, {},
		{make_shared<MagicVariableDeclaration>("gasleft", TypeProvider::get().functionType(strings(), strings{"uint256"}, FunctionType::Kind::GasLeft))}
	);
